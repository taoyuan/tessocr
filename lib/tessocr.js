...
Tess.prototype.segment = function (image, options, cb) {
  ...
  Jimp.read(image, function (err, image) {
    var result = {
      dimensions: [image.bitmap.width, image.bitmap.height],
      boxes: []
    };
    var boxes = [];
    var line = 0;
    ...
    image.scan(0, 0, image.bitmap.width, image.bitmap.height, function (x, y, idx) {
      var dot = isDot(this.bitmap.data, idx);
      if (y === line) {
        if (blank && dot) {
  ...
  function isDot(data, idx) {
    return (data[ idx + 0 ] < 0x7F || data[ idx + 1 ] < 0x7F || data[ idx + 2 ] < 0x7F) && data[ idx + 3 ] > 0;
  }
};
 ...
 * @param {Number} [options.psm] page segment mode
 * @param {Array} [options.rects] Multi image rects
 * @param {Number} [options.jobs] Jobs to run recognize. It is work only when has rects
 * @param {Function} [cb] Callback
 */
Tess.prototype.ocr = function (image, options, cb) {
  ...
  if (!Array.isArray(rects)) {
    return this.recognize(image, options, cb);
  }

  var jobs = utils.arrange(rects, options.jobs || os.cpus());

  var that = this;
  async.map(jobs, function (job, cb) {
    options.rects = jobTransform(job);
...
function parseOptions(options) {
  options = options || {};
  if (!options.language ) {
    options.language = options.language || options.l;
  }
...
function jobTransform(job) {
  return job.map(function (rect) {
