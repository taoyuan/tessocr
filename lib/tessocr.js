...
Tess.prototype.ocr = function (image, options, cb) {
  console.log('tess-ocr begin');
  console.time('tess-ocr');
  if (typeof options === 'function') {
    cb = options;
    options = cb;
  }
  ...
  if (sopts) {
    ...
    console.time('segline');
    if (bitmap) {
      seglineAndRecognize(bitmap, bitmap.bytesPerPixel);
    } else if (typeof image === 'string' || Buffer.isBuffer(image)) {
      Jimp.read(image, function (err, image) {
        seglineAndRecognize(image.bitmap);
      });
    } else {
  ...
  function seglineAndRecognize(bitmap, channels) {
