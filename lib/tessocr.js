...
Tess.prototype.ocr = function (image, options, cb) {
    ...
    Jimp.read(image, function (err, image) {
      if (rects) {
        rects = _.flatten(_.map(rects, function (rect) {
          sopts.rect = rect;
          return segline(image.bitmap.data, image.bitmap.width, image.bitmap.height, sopts);
        }));
      } else {
        rects = segline(image.bitmap.data, image.bitmap.width, image.bitmap.height, sopts);
      }
  ...
  function done(err, result) {
    if (err) return cb(err);
    cb(null, Array.isArray(result) ? result.join('\n') : result);
  }
};
